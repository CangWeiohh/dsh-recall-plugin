 ...
 * - 运行时文件必须进包（lib/、cordis.patch.yml、README、LICENSE）；
 * - 仓库开发文件绝不进包（AGENTS.md / docs/ / tests/ —— AGENTS.md 已在
 *   .gitignore 中确认不进 npm，这里从 pack 输出侧再兜一道）。
...
describe('npm 发布包内容', () => {
  ...
  it('仓库开发文件不进包（AGENTS.md / docs / tests）', () => {
    for (const disallowed of ['AGENTS.md', 'docs/', 'tests/', 'reference/']) {
