...
describe('buildReport', () => {
  ...
  it('全部一致：exit 0 且无差异标记', () => {
    const r = buildReport({ local: '0.1.1-rc.2', mirror: '0.1.1-rc.2', latest: '0.1.1-rc.2', peers })
  ...
  it('镜像漂移：本地 ≠ reference 记录，输出重拉提醒并 exit 1', () => {
    const r = buildReport({ local: '0.1.2', mirror: '0.1.1-rc.2', latest: '0.1.1-rc.2', peers })
  ...
  it('peer 越界：npm 最新 dsh 超出 peer 范围，输出扩范围提醒并 exit 1', () => {
    const r = buildReport({ local: '0.1.1-rc.2', mirror: '0.1.1-rc.2', latest: '0.2.0', peers })
    expect(r.ok).toBe(false)
    expect(r.exitCode).toBe(1)
  ...
  it('本地实装版本越界：按子包报 ✗', () => {
    ...
    const r = buildReport({ local: '0.1.1-rc.2', mirror: '0.1.1-rc.2', latest: '0.1.1-rc.2', peers: bad })
    expect(r.ok).toBe(false)
  ...
  it('peer 包未装于全局/profile 树：跳过实装校验，不算差异', () => {
    ...
    const r = buildReport({ local: '0.1.1-rc.2', mirror: '0.1.1-rc.2', latest: '0.1.1-rc.2', peers: uninstalled })
    expect(r.ok).toBe(true)
    expect(r.lines.join('\n')).toContain('跳过实装校验')
  })
  ...
  it('无法解析的范围：警告并跳过，不误报差异', () => {
    ...
    const r = buildReport({ local: '0.1.1-rc.2', mirror: '0.1.1-rc.2', latest: '0.1.1-rc.2', peers: weird })
    expect(r.ok).toBe(true)
    expect(r.lines.join('\n')).toContain('无法解析')
  })
  ...
  it('非 dsh 线 peer（cordis/schemastery 独立版本线）：不做 npm 最新对比，仅本地实装校验', () => {
    ...
    const r = buildReport({ local: '0.1.1-rc.2', mirror: '0.1.1-rc.2', latest: '0.1.1-rc.2', peers: cordis })
    expect(r.ok).toBe(true)
  ...
  it('找不到本地 dsh：✗ 且 exit 1', () => {
    const r = buildReport({ local: null, mirror: '0.1.1-rc.2', latest: '0.1.1-rc.2', peers: [] })
    expect(r.ok).toBe(false)
    expect(r.lines.join('\n')).toContain('未找到本地已装 dsh')
  })

  it('reference 未记录归档版本：⚠ 提醒且 exit 1（无法确认一致）', () => {
    const r = buildReport({ local: '0.1.1-rc.2', mirror: null, latest: '0.1.1-rc.2', peers: [] })
    expect(r.ok).toBe(false)
    expect(r.lines.join('\n')).toContain('未记录「归档 dsh 版本」')
  })

  it('npm 最新获取失败（离线）：降级为仅本地比对，不误报差异', () => {
    const r = buildReport({ local: '0.1.1-rc.2', mirror: '0.1.1-rc.2', latest: null, peers: [] })
    expect(r.ok).toBe(true)
    expect(r.lines.join('\n')).toContain('无法获取 npm 最新 dsh')
  })

  it('npm 最新版比本地新：提示可升级（不 fail，镜像基准由漂移层捕获）', () => {
    const r = buildReport({ local: '0.1.1-rc.2', mirror: '0.1.1-rc.2', latest: '0.1.2', peers })
    expect(r.ok).toBe(true)
