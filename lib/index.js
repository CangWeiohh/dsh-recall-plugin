...
export function apply(ctx, config) {
  ...
  const endpoints = {
    ...
    'manage': async (args) => {
      ...
      if (op === 'deleteAll') {
        // 清空全部工作区的全部快照：遍历所有 stores，用 git tag -l 'snap-*'
        // 直接列举实际存在的 tags（不依赖 index.json 的 entries，因为
        // index.json 可能是空的或过期的），批量删除 → 清空内存 snapshots
        // → 写空 index.json。全部在串行队列里跑，与快照/gc 互斥；单 store
        // 失败只记错误继续，tag 残留由下次清理或 gc 幂等收尾。
        const allRoots = Array.from(state.stores.keys())
        ...
        await enqueue(async () => {
          for (const root of allRoots) {
            ...
            try {
              if (!state.gitExe) continue
              // 用 git tag -l 'snap-*' 列举实际存在的 tags
              const listOut = await rt.runShell(rt.scripts.listTagsScript(store, state.gitExe), { timeoutMs: 60000, stdoutMaxBytes: 8388608 })
              const tags = rt.scripts.stripBom(listOut).trim().split('\n').map((ln) => ln.trim()).filter(Boolean)
              ...
              for (const [id, snap] of state.snapshots.entries()) {
                if (snap.root === root) toDelete.push(id)
              }
              for (const id of toDelete) state.snapshots.delete(id)
              // 写空 index.json（loadIndex 先补齐，避免覆盖误留磁盘条目）
