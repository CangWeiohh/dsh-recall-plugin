...
export function createRuntime(ctx, config) {
  ...
  async function resolveRoot(sessionId) {
    ...
    // 只有这类结果才允许缓存。回退到 sandboxPolicy.workspaceRoot 的临时
    // 结果不缓存——它通常是 harness 启动目录，会话稍后变 live/持久化后
    // 若仍被旧错误根遮蔽，撤回按钮将永不出现（只能靠手动刷新）。
    ...
    if (!root && sessionId) {
      ...
      // 解析 cwd，避免回退到 workspaceRoot（harness 启动目录）而查错 store。
      // 用 sessionQuery.listSessions 只做廉价目录枚举，不读全量日志。
      try {
