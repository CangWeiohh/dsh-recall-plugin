...
- 「全部删除」功能失效：1.5.1 版本依赖 `index.json` 的 `entries` 枚举快照 ID，当索引文件为空时跳过整个 store，导致实际存在的 git tags 未被删除。改为直接用 `git tag -l 'snap-*'` 列举每个 store 的实际 tags，不再依赖可能过期或为空的索引文件；批量删除后清空内存条目并写空索引，确保彻底清除。

## [1.5.1] - 2026-08-19

### 新增

- 「快照管理」卡片新增「全部删除」按钮：一次性清空所有工作区的全部快照（与列表的跨工作区范围一致）。点按后先二次确认（显示总条数、强调不可恢复），确认后 Host 端以磁盘 dump 枚举全部 store，逐个批量删除 `snap-*` tag、清空索引并失效列表缓存；与单条删除/快照/gc 共用同一串行队列，互斥无 git 锁竞态。
