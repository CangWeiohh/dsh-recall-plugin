...
- **Interplay with dsh-routing-suite (progressive tool-disclosure router)**: if you also run the router-standard preset of dsh-routing-suite, a recall forks a new session via `sessions.fork`, while the router persists its stage state per session id (`$DSH_HOME/router-standard/stages.json`, or `~/.dsh/router-standard/stages.json` when unset) — the forked session has no record, so the router stage resets to its default (the tool surface temporarily narrows). dsh-routing-suite ≥ v1.20.3 resolves this: the router reads the official `SessionHeader.parentSession` in `agent/pre-step`, and a forked session automatically inherits the parent's stage (`stage`/`guided`/`stageAtTime`); no change to this plugin is needed, and it takes effect after restarting DSH. On older versions without inheritance, you can manually call `phase_advance` in the new session to restore the stage. This plugin does not depend on that router — the two work independently.
